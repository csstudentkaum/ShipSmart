...
trait Request
{
    ...
    private function send($method){
        ...
        // curl_close is deprecated in PHP 8.5 as it's a no-op; avoid calling it.
        // Report errors if any
        if ($err) {
            throw new TrackingMoreException("failed to request: $err");
        }

        // If response empty or not valid JSON, log debug info to PHP error log for investigation
        $decoded = json_decode($response, true);
        if ($response === false || $response === null || (json_last_error() !== JSON_ERROR_NONE && empty($decoded))) {
            error_log("TrackingMore debug: HTTP status=" . ($info['http_code'] ?? 'unknown') . " url=" . $this->url . " response=" . var_export($response, true));
        }
